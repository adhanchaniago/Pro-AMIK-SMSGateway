...
$getAbsensi = $db->getOneDetailAbsensi($_GET['id']);

foreach ($getAbsensi as $row) {
     if ($row->detail_status != 0) {

          $dAbsensi = $db->getOneAbsensi($_GET['id']);
          $dMhs = $db->getOneMahasiswa($row->mahasiswa_id);
...
location='index.php?page=pages/abensi/detail&id=$_GET[id];
</script>";
